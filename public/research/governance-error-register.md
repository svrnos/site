...
> A classification system for AI platform governance failures, modeled on HTTP status codes. Twenty-two structurally distinct failure modes and success states across five tiers.
...
11. [Contribute to the Register](#contribute)
12. [References](#references)

## Abstract

AI safety discourse has a vocabulary problem. Failure modes at the governance layer, the policies, escalation paths, audit systems, and human review processes that wrap AI models, are poorly named, inconsistently described, and not shared across the engineering, legal, policy, and research communities that need to communicate about the same events. This paper introduces the SVRNOS AI Governance Error Taxonomy: a structured classification system for AI platform governance failures organized by numeric codes derived from HTTP protocol error semantics. The taxonomy covers twenty-one structurally distinct failure modes and success states across five tiers: pre-infrastructure, success states, structural moves, operator errors, and infrastructure failures. Three codes extend beyond standard HTTP where no existing code maps cleanly to the governance failure. The taxonomy is a signaling layer, not a policy layer, codes describe what failed, they do not define what platforms are required to do. The register is open and extensible. SVRNOS maintains it. This is v0.1.
...
| 205 | Reset Content | Generation completed and response was delivered or visible, then retroactively suppressed by a downstream classifier without explanation. From the operator's perspective: no audit trail of what was generated, no signal to the user about why, no path to appeal. | Not 403, which refuses upfront before generation. Not 210, which suppresses the audit trail of a governed action. 205 is specifically the case where generation succeeded and the output was then unmade, the system fulfilled the request and reset it post-hoc. Sits in 2xx because the harmful output was caught and removed, a governance action that succeeded in outcome. The failure is in the mechanism: no audit trail, no user signal, no path to appeal. | DocumentedSVRNOS taxonomy production, April 27, 2026. Meta's Llama streamed a governance research response, then retroactively suppressed it via a downstream classifier. The generation was visible before suppression. No audit trail was produced. [See case study →](/insights/ger-205) |
...
| 403 | Policy Not Enforced | Platform knows a user or use case is prohibited, but enforcement is not wired into the product. The prohibition exists in policy; it does not exist in production. | Not 401, where identity is unverified. 403 is known prohibition with no enforcement mechanism, the platform knows and does not act. | IllustrativeAccount flagged as underage still accesses adult-coded AI features because policy and product permissions are not synchronized. |
...
| 428 | Subject Preconditions Not Required | AI system produces personalized content involving a named third party (specific real person, identified by name or identifier) without requiring the requester to attest to or demonstrate preconditions necessary for safe production: consent of the named subject, relational authority, identity ownership, or age verification of the subject. The system processed the request as if the third party did not need protection. | Not 401, where the *requester* lacks identity verification. Not 511, where the requester's verification gate was never installed at session start. Not 404, where no rule exists for the harm category. 428 is a structural failure to require preconditions about the *subject* of the content before producing personalized content involving them. | IllustrativeA user asks a chatbot to rewrite a popular song's lyrics with a named classmate substituted as the subject of the song's abusive content, then formats the result for forum posting. The system produces the personalized output. No requirement to attest to the subject's consent, identity, or age was enforced before generation. |
...
| 451\* | Jurisdiction Evasion | Platform routes high-risk model inference to a regulation-free zone to bypass audit obligations and governance requirements by design. | Not 301, which is a governed permanent move. 451 is evasion of oversight by design, the routing exists to avoid legal accountability, not to improve governance. | IllustrativeEU-based AI company routes restricted queries to a Caribbean subsidiary to avoid GDPR-AI Act audit requirements. |
...
**Register completeness.** Twenty-two codes do not exhaust the space of AI governance failures. The multi-model synthesis process achieved coverage consensus across four frontier models; coverage gaps in that process are coverage gaps in the taxonomy. Significant failure modes are likely absent from v0.1.
